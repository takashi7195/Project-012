  ...
  return (
    with dates as (
      ...
      select coalesce(jsonb_agg(jsonb_build_object(
        ...
        'status',case when r.current_batch_id is null then 'missing_head'
    ...
    select jsonb_build_object(
      ...
      'warnings',coalesce((select jsonb_agg(distinct status order by status) from (
        select case when current_batch_id is null then 'missing_head' when batch_state not in ('ready','published') then 'not_published' when result_count<race_count then 'result_incomplete' else null end status from rows
