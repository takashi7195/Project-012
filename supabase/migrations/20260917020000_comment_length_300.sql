...
alter table public.comments add constraint comments_body_length check (char_length(body) between 1 and 300);

-- Allow up to 300 characters in user comments.
...
begin
  if p_reply_source is null
     ...
     or pg_catalog.char_length(pg_catalog.btrim(p_ai_reply)) < 1
     or p_body is null
     or pg_catalog.char_length(pg_catalog.btrim(p_body)) not between 1 and 300
