-- Record whether AI Takashi's reply came from the existing template set,
-- Gemini, or the empathetic fallback for sensitive comments.

begin;

alter table public.comments
  add column if not exists reply_source text not null default 'template'
  check (reply_source in ('template', 'gemini', 'empathetic'));
...
declare
  v_comment_id uuid;
begin
  if p_reply_source not in ('gemini', 'empathetic')
